...
export async function buildExport(entity:ExportEntity,format:"csv"|"json"="csv"){
 ...
 const spec=map[entity]; let q=supabase.from(spec.table).select(spec.select).eq("business_id",business.id); if(spec.order) q=q.order(spec.order,{ascending:false}); const {data,error}=await q.limit(10000); if(error) throw new Error(error.message); const rawRows: unknown=data??[]; const rows=Array.isArray(rawRows)?rawRows.filter(isRecord):[];
