...
export async function exportBusinessData(groups: string[], format: 'json'|'csv'|'xlsx', selectedTables?: string[]) {
 ...
 const {data,error}=await db.rpc('tenh_export_business',{p_business_id:business.id,p_tables:tables});
 ...
 const content=format==='xlsx'?await exportWorkbook(cleaned):format==='csv'?exportCsv(cleaned):JSON.stringify({version:2,businessId:business.id,exportedAt:new Date().toISOString(),scope:'All branches of this business',excluded:'Login credentials, security tokens, platform administration and binary uploads. File references are included.',tables:Object.fromEntries(Object.entries(cleaned).map(([key,rows])=>[key,rows.length])),data:cleaned},null,2);
 const filename=`tenh-business-${new Date().toISOString().slice(0,10)}.${format}`;
 const {data:{user}}=await db.auth.getUser();
 if(user) await db.from('data_transfer_jobs').insert({business_id:business.id,user_id:user.id,direction:'export',entity:'all',format,filename,row_count:rowCount,status:'completed',summary:{groups,tables}});
