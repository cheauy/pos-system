...
export default async function BundleItemsPage() {
  ...
  const [products, stock, items, categories, groups, options, canCreate, canPack, canEdit, canDelete] = await Promise.all([
    rows(db.from('branch_products').select('id,name,sku,description,category_id,image_url,variant_image_url,size,color,product_type,bundle_stock_mode,cost_price,selling_price,is_active,is_online,is_pos,updated_at').eq('business_id', business.id).order('id')),
    ...
    rows(db.from('branch_bundle_items').select('id,bundle_product_id,component_product_id,quantity,selected_options').eq('business_id', business.id).order('id')),
