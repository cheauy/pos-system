...
export async function loadMemberPermissions(businessId:string,memberId:string) {
 ...
 const {data:m,error}=await supabaseAdmin.from('business_members').select('id,role,team_revision').eq('id',memberId).eq('business_id',businessId).maybeSingle();
 if(error||!m)throw new Error('User not found.');
 const baseline=await getRolePermissions(businessId,m.role as BusinessRole);
