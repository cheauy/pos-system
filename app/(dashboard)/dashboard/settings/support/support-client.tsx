...
type Report = { id: string; title: string; description: string; status: string; created_at: string };
  ...
  return <main className="mx-auto max-w-[1600px] space-y-5 pb-8">
    ...
    <div className="grid items-start gap-5 xl:grid-cols-[1fr_1.1fr]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 sm:p-6"><h2 className="font-bold">New report</h2><form onSubmit={submit} className="mt-5 space-y-4"><fieldset disabled={busy} className="space-y-4 disabled:opacity-60">
        ...
        <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-semibold">Priority<select name="priority" defaultValue="normal" className={field}><option value="low">Low</option><option value="normal">Normal</option><option value="urgent">Urgent</option></select></label><label className="block text-sm font-semibold">Page path<input name="pagePath" maxLength={500} placeholder="/dashboard/orders" className={field} /></label></div>
        <label className="block text-sm font-semibold">What happened? *<textarea name="description" required minLength={10} maxLength={6000} rows={6} placeholder="What were you doing? What did you expect, and what happened instead?" className={field} /></label><p className="text-xs leading-5 text-slate-500">Include steps to reproduce the problem. Do not include passwords, API keys or payment details.</p>
