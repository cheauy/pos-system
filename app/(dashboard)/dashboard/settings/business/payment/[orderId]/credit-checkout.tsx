...
import { ArrowLeft, CheckCircle2, Link2, Store } from "lucide-react";
...
export type CreditCheckoutOrder={
 ...
 manual_bank_name:string|null;manual_account_name:string|null;manual_account_number:string|null;manual_qr_image_url:string|null;payment_expires_at:string|null;
};
export default async function CreditCheckout({order,businessId}:{order:CreditCheckoutOrder;businessId:string}){
 ...
 return <main className="mx-auto w-full max-w-[1540px] pb-10">
  ...
  <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_400px]">
   ...
   <section className="rounded-3xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-900">
    ...
    {paid?<div className="mt-4 space-y-3"><CheckCircle2 className="text-emerald-600"/><p>Business mode · Available {credits?.modeCredits}</p><p>Store URL · Available {credits?.urlCredits}</p><p className="text-sm text-slate-500">Your purchased credits never expire.</p><Link href="/dashboard/settings/business" className="inline-flex rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white">Open Business Details</Link></div>:submitted?<div className="mt-4"><p className="font-semibold text-amber-700">Awaiting payment approval</p><p className="mt-2 text-sm text-slate-500">Your receipt is submitted. Credits will appear here after approval.</p></div>:pending?(payway?<div className="mt-4 space-y-4"><PaywayCheckoutButton orderId={order.id} kind="business_change"/><p className="text-xs text-slate-500">The payment window is 10 minutes. Purchased credits have no expiry.</p><form action={cancelBusinessChangeCheckout}><input type="hidden" name="orderId" value={order.id}/><button className="w-full rounded-xl border px-4 py-2 text-sm font-semibold">Cancel checkout safely</button></form></div>:<PaymentMethodSelector orderId={order.id} kind="business_change" manualAvailable={manualAvailable} initialMethod={null} manualContent={manualContent}/>):<div className="mt-4 space-y-3"><p className="font-semibold">{order.status==='under_review'?'Payment needs review':'Checkout closed'}</p><p className="text-sm text-slate-500">{order.review_note||'No credits were added. Return to Business Details to continue.'}</p><Link href="/dashboard/settings/business" className="text-sm font-bold text-blue-600">Back to Business Details</Link></div>}
   </section>
  </div>
 </main>;
}
