...
import { Package, Search, Star, Eye, EyeOff, TriangleAlert, RefreshCw, List, LayoutGrid, Pencil } from "lucide-react";
...
export default function CatalogManager({ products, categories, currency, canEdit, storeUrl, featuredIds = [] }: {
  ...
  const star = (product: ManagedProduct) => <button type="button" aria-label={`Feature ${product.name} ${product.sku ?? ""}`} aria-pressed={featured.has(product.id)} disabled={!canEdit || pending} onClick={() => bulk("featured", !featured.has(product.id), [product.id])} className="rounded p-2 text-amber-500 disabled:opacity-40"><Star size={16} fill={featured.has(product.id) ? "currentColor" : "none"} /></button>;
  return <section className="bg-white p-4 sm:p-6" id="catalog" aria-busy={pending}>
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><p className="text-xs text-slate-500">Online Store</p><h2 id="catalog-dialog-title" className="text-xl font-bold text-slate-900">Storefront Products</h2><p className="mt-1 text-xs text-slate-500">Control your online catalog. Photos, prices and stock stay synced with inventory.</p></div>
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      ...
      <select aria-label="Sort products" value={sort} onChange={e => filter(setSort, e.target.value)} className={control}><option value="featured">Featured first</option><option value="name">Name</option><option value="price-low">Price: low to high</option><option value="price-high">Price: high to low</option></select>
    ...
    <div className="my-3 flex flex-wrap items-center gap-2 text-xs"><label className="mr-2 flex items-center gap-2"><input type="checkbox" aria-label="Select current page" checked={shown.length > 0 && shown.every(p => selected.includes(p.id))} disabled={!canEdit || pending} onChange={e => setSelected(e.target.checked ? shown.map(p => p.id) : [])} />{selected.length ? `${selected.length} selected` : "Select page"}</label>{[{ label: "Show online", field: "visibility", value: true }, { label: "Hide", field: "visibility", value: false }, { label: "Feature", field: "featured", value: true }, { label: "Remove feature", field: "featured", value: false }].map(item => <button key={item.label} type="button" className={control} disabled={!canEdit || pending || !selected.length} onClick={() => bulk(item.field as "visibility" | "featured", item.value)}>{item.label}</button>)}</div>
    {grid ? <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">{shown.map(product => <article key={product.id} className="rounded-xl border border-slate-200 p-3"><div className="mb-2 flex items-center justify-between"><input aria-label={`Select ${product.name} ${product.sku ?? ""}`} type="checkbox" disabled={!canEdit || pending} checked={selected.includes(product.id)} onChange={e => select(product.id, e.target.checked)} />{badge(product)}{star(product)}</div>{photo(product)}<Link className="mt-2 block text-sm font-semibold" href={`/dashboard/products/${product.id}/edit`}>{product.name}</Link><p className="my-1 text-xs text-slate-500">{[product.sku, product.size, product.color].filter(Boolean).join(" / ")}</p><div className="flex items-center justify-between text-xs"><span>{money(product.selling_price)} / {product.stock_quantity} in stock</span>{online(product)}</div></article>)}</div> : <div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="bg-slate-50 text-slate-500"><tr>{["", "Product", "Category", "Price", "Stock", "Status", "Online", "Featured", "Actions"].map((label, index) => <th key={index} className="whitespace-nowrap p-2">{label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{shown.map(product => <tr key={product.id}><td className="p-2"><input aria-label={`Select ${product.name} ${product.sku ?? ""}`} type="checkbox" disabled={!canEdit || pending} checked={selected.includes(product.id)} onChange={e => select(product.id, e.target.checked)} /></td><td className="p-2"><div className="flex min-w-44 items-center gap-2">{photo(product)}<div><Link className="font-semibold" href={`/dashboard/products/${product.id}/edit`}>{product.name}</Link><p className="text-[10px] text-slate-500">{[product.sku, product.size, product.color].filter(Boolean).join(" / ")}</p>{product.is_online && !isVisible(product) && <p className="text-amber-600">Hidden by category</p>}</div></div></td><td className="p-2"><span className="rounded-full bg-blue-50 px-2 py-1 text-[10px] text-blue-700">{categoryMap.get(product.category_id ?? "")?.name ?? "Uncategorized"}</span></td><td className="whitespace-nowrap p-2">{money(product.selling_price)}</td><td className="p-2">{product.stock_quantity}</td><td className="p-2">{badge(product)}</td><td className="p-2">{online(product)}</td><td className="p-2">{star(product)}</td><td className="p-2"><Link aria-label={`Edit ${product.name} ${product.sku ?? ""}`} href={`/dashboard/products/${product.id}/edit`}><Pencil size={14} /></Link></td></tr>)}</tbody></table></div>}
