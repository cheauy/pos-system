...
export type ExtendSubscriptionState = {
  success: boolean;
  message: string;
  submittedAt: number
};
...
export async function extendBusinessSubscription(
  _previousState: ExtendSubscriptionState,
  formData: FormData,
  ...
  try {
    ...
    const confirmationText =
      typeof confirmationTextValue === "string"
        ? confirmationTextValue.trim()
        : "";
    ...
    /*
     * Suspended businesses must be restored
     * before changing their subscription.
     */
    if (
      business.disabled_reason ===
      "manually_suspended"
    ...
    /*
     * An expired business must use the
     * Reactivate workflow, not Extend.
     */
    if (isExpired) {
      throw new Error(
        "This subscription has expired. Use Reactivate instead.",
      );
    }

    if (!business.is_active) {
      throw new Error(
        "Only active businesses can extend their subscription.",
      );
    }
    ...
    const previousMaxStaff = Number(
      business.max_staff ?? 3,
    );

    /*
     * Active subscriptions extend from the
     * current expiry date.
     */
    const newExpiry =
  ...
  months;
    ...
    if (updateError) {
      throw new Error(
        `Unable to extend subscription: ${updateError.message}`,
      );
    }
  ...
  await supabaseAdmin
    ...
    .insert({
      ...
      new_expiry:
        newExpiryIso,
      reason:
        `Subscription extended by ${months} ${
          months === 1
            ? "month"
            : "months"
        }`,
    ...
    if (historyError) {
      throw new Error(
        `Subscription was extended, but history could not be recorded: ${historyError.message}`,
      );
    }
