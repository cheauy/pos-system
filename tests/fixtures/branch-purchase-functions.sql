...
begin
  ...
  if v_order_status not in (
    ...
    raise exception
      'Only new, pending, or completed orders can be cancelled.';
  end if;
...
begin
    ...
    if v_purchase_status = 'cancelled' then
        raise exception 'Purchase has already been cancelled.';
    end if;

    if v_purchase_status <> 'received' then
        raise exception 'Only received purchases can be cancelled.';
    end if;
    ...
    loop
        ...
        if v_current_stock < v_item.quantity then
            raise exception
                'Cannot cancel purchase. Product "%": current stock is %, cancellation requires %.',
                v_item.product_name,
                v_current_stock,
                v_item.quantity;
        end if;
...
$function$
;
CREATE OR REPLACE FUNCTION public.create_order_return(p_order_id uuid, p_reason text, p_items jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
declare
  ...
  v_new_remaining_balance numeric(12, 2) := 0;
  v_new_change_amount numeric(12, 2) := 0;
begin
  v_user_id := auth.uid();
  ...
  /*
   * Load the order and current business.
   */
  select
    o.business_id,
    o.status
  ...
  /*
   * Check active business membership.
   */
  if not exists (
    select 1
    from public.business_members bm
  ...
  if v_order_status not in (
    ...
    raise exception
      'Only new, pending, or completed orders can be returned.';
  end if;
  ...
  loop
    ...
    if v_remaining_quantity <= 0 then
      raise exception
        'Product "%" has already been fully returned.',
        v_product_name;
    end if;

    if v_return_quantity >
      v_remaining_quantity
    then
      raise exception
        'Cannot return product "%". Sold: %, already returned: %, maximum remaining return: %.',
        v_product_name,
        v_sold_quantity,
        v_already_returned,
        v_remaining_quantity;
    ...
    /*
     * return_items currently uses owner_id.
     * Add business_id here only if your table has it.
     */
    ...
    values (
      ...
      v_stock_before,
      v_stock_after,
      'Returned from order. Return number: ' ||
      v_return_number ||
      '. Reason: ' ||
      trim(p_reason)
    );
  ...
  /*
   * Recalculate subtotal after returns.
   */
  select
    coalesce(
      sum(
  ...
  left join (
    ...
    on returned.order_item_id = oi.id
  where oi.order_id = p_order_id;
  ...
  if not found then
    raise exception
      'Order was not found while recalculating totals.';
  end if;
  ...
  if not found then
    raise exception
      'Unable to update the order totals.';
  end if;
...
begin
  ...
  if not exists (
    ...
    where business_id = v_po.business_id and user_id = v_user and is_active = true
      and role in ('owner','admin','manager')
  ) then raise exception 'You do not have permission to receive stock.'; end if;
  ...
  loop
    ...
    insert into public.stock_adjustments (
      ...
      v_before, v_after, 'Purchase order receipt', v_po.po_number, v_user
    );
...
$function$
;
CREATE OR REPLACE FUNCTION public.tenh_create_order_return_accounted(p_order_id uuid, p_reason text, p_items jsonb, p_refund_method text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO 'public'
AS $function$
