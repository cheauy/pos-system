...
function button(content,fonts=Promise.resolve()) {
 ...
 const deps={react:{useRef:()=>({current:false}),useState:initial=>[initial,value=>{if(typeof value==='string'&&value)errors.push(value);}]},'lucide-react':{Printer:()=>null}};
...
test('oversize shipping label blocks printing instead of clipping address',async()=>{const b=button({querySelectorAll:selector=>selector==='img'?[]:[{dataset:{widthMm:'80',heightMm:'50'},getBoundingClientRect:()=>({width:320,height:300})}]});await b.click();assert.equal(b.prints(),0);assert.match(b.errors[0],/too long/);});
