...
function setup(fail=false){
 ...
 const deps={'next/server':{NextResponse:{json:(body,options)=>({body,status:options?.status??200})}},'@/lib/supabase/admin':{supabaseAdmin:admin},'@/lib/tenancy/domain':{normalizeTenantSlug:v=>v,getTenantSlugFromHost:()=>null},'@/lib/storefront/checkout-validation':validation};
...
const checkout={items:[{productId:'product',quantity:1}],guestName:'Customer',guestPhone:'012345678',guestAddress:'Street',fulfillmentType:'delivery',paymentMethod:'khqr'};
...
test('proof is privately uploaded and attached to the transactional order call',async()=>{
 const ctx=setup();assert.equal((await ctx.post(request(png()),{params:Promise.resolve({slug:'shop'})})).status,200);assert.equal(ctx.calls.rpc,1);assert.equal(ctx.calls.args.p_payment_reference,`proof:${ctx.calls.uploads[0]}`);assert.equal(ctx.calls.removed.length,0);
});
