...
export function PosReceipt({receipt:r,context}:{receipt:SaleReceipt;context?:ReceiptContext}) {
 ...
 return <article className={`${s.receipt} ${s.classic} receipt`} data-paper={a.paperSize} data-density={a.density} style={{"--receipt-font-scale":printTextScale(a.fontSize),"--receipt-alignment":a.alignment} as CSSProperties}>
  ...
  <header>
   ...
   {a.showBusinessName!==false && <h2>{r.businessName || context?.store.name}</h2>}
   {a.showAddress && context?.store.address && <p className={`${s.pre} ${s.contact}`}>{context.store.address}</p>}
   {a.showPhone && context?.store.phone && <p className={s.contact}>Tel: {context.store.phone}</p>}
   {a.showWifi && a.wifiPassword && <p className={s.contact}>Wi-Fi password: {a.wifiPassword}</p>}
  ...
  <Separator/>
  <table><thead><tr><th>#</th><th>Item</th><th>Qty</th><th>Price</th><th>Total</th></tr><tr aria-hidden="true"><td colSpan={5} className={s.separatorCell}><Separator/></td></tr></thead><tbody>{r.lines.map((l,i)=><tr key={i}><td>{i+1}</td><td>{l.name}{l.variant && <small>{l.variant}</small>}{l.options?.map((o,j)=><small key={j}>{o.groupName?`${o.groupName}: `:''}{o.name}</small>)}</td><td>{l.quantity}</td><td>{cash(l.unitPrice)}</td><td>{cash(l.subtotal)}</td></tr>)}</tbody></table>
  <Separator/>
